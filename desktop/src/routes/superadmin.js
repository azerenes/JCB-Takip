...
const LicenseActivation = require('../database/models/LicenseActivation');
...
router.get('/tenants/:id/stats', sa, async (req, res) => {
    try {
        ...
        const [deviceCount, userCount, alertCount] = await Promise.all([
            ...
            deviceIds.length ? require('../database/models/Alert').countDocuments({ deviceId: { $in: deviceIds } }) : 0
        ]);
